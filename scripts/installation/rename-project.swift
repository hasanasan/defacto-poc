...
class XcodeProjectRenamer: NSObject {
    ...
    func run() {
        ...
        let scriptPath = URL(fileURLWithPath: currentPath)
    ...
    private func findOldProjectName(_ projectFolderPath: URL) -> String? {
        do {
            let directoryContents = try FileManager.default.contentsOfDirectory(at: projectFolderPath,
                                                                                includingPropertiesForKeys: nil, options: [])
            guard let projectFileName = directoryContents.filter({ $0.pathExtension == "xcodeproj" }).first?
        ...
        do {
            ...
            if oldItemName.contains(oldName) {
                ...
                let newItemName = oldItemName
                    .replacingOccurrences(of: oldName.replacingOccurrences(of: "-", with: "_"),
